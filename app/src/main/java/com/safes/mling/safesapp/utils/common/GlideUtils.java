...
public class GlideUtils {
    ...
    public void LoadContextBitmap(Context context, String path, ImageView imageView, int errorid, String bitmapOrgif) {
        ...
        if (!TextUtils.isEmpty(path) && path.startsWith("https://")|| path.startsWith("http://")) {
            if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
                if (context != null && imageView != null)
                    if (Util.isOnMainThread()) {
                        Glide.with(context).load(path).placeholder(errorid).error(errorid).crossFade().into(imageView);
                    }
            ...
            if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
                if (context != null && imageView != null)
                    if (Util.isOnMainThread()) {
                        Glide.with(context).load(url).placeholder(errorid).error(errorid).crossFade().into(imageView);
                    }
            } else if (bitmapOrgif.equals(LOAD_GIF)) {
                if (context != null && imageView != null)
                    if (Util.isOnMainThread()) {
                        Glide.with(context).load(url).asGif().crossFade().into(imageView);
                    }
            }
        }
    }
     ...
     * @param bitmapOrgif 加载普通图片 或者GIF图片 ，GIF图片设置bitmap显示第一帧
     */
    public void LoadFragmentBitmap(Fragment fragment, File path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            if (path != null)
                Glide.with(fragment).load(path).placeholder(placeid).error(errorid).crossFade().into(imageView);
        } else if (bitmapOrgif.equals(LOAD_GIF)) {
     ...
     * @param bitmapOrgif 加载普通图片 或者GIF图片 ，GIF图片设置bitmap显示第一帧
     */
    public void LoadBitmap(Context context, File path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            if (path != null)
    ...
    public void LoadSupportv4FragmentBitmap(Fragment fragment, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        ...
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            if (!TextUtils.isEmpty(path) && path.startsWith("http://")) {
                ...
                Glide.with(fragment).load(url).placeholder(placeid).error(errorid).crossFade().into(imageView);
            }
        } else if (bitmapOrgif.equals(LOAD_GIF)) {
            if (!TextUtils.isEmpty(path) && path.startsWith("http://")) {
                Glide.with(fragment).load(path).placeholder(placeid).error(errorid).crossFade().into(imageView);
     ...
     * @param imageView
     */
    public void LoadContextCircleBitmap(Context context, String path, ImageView imageView, int defe) {
        String url = Constants.BASE_URL + path;
        if (!TextUtils.isEmpty(path) && path.startsWith("http://")) {
            if (context != null && imageView != null)
                if (Util.isOnMainThread()) {
                    Glide.with(context).load(path).placeholder(R.mipmap.ic_launcher_round).error(defe).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
                }
        } else {
            if (context != null && imageView != null) {
                if (Util.isOnMainThread()) {
                    Glide.with(context).load(url).placeholder(R.mipmap.ic_launcher_round).error(defe).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
                }
            }
        }
    }
    ...
    public void bitContextCircleBitmap(Context context, int path, ImageView imageView, int def) {
        Glide.with(context).load(path).error(def).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
    }

    public void bitContextCircleBitmap(Context context, Bitmap path, ImageView imageView, int def) {
        Glide.with(context).load(path).error(def).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
    }
     ...
     * @param imageView
     */
    public void LoadfragmentCircleBitmap(Fragment fragment, String path, ImageView imageView, int defe) {
        String url = Constants.BASE_URL + path;
        if (!TextUtils.isEmpty(path) && path.startsWith("http://")) {
            Glide.with(fragment).load(path).placeholder(R.mipmap.ic_launcher_round).error(R.mipmap.ic_launcher_round).bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
        } else {
            Glide.with(fragment).load(url).placeholder(R.mipmap.ic_launcher_round).error(R.mipmap.ic_launcher_round).bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
        }
    }
     ...
     * @param imageView
     */
    public void LoadfragmentCircleBitmap(Fragment fragment, File path, ImageView imageView, int defe) {
        if (path != null)
            Glide.with(fragment).load(path).placeholder(R.mipmap.ic_launcher_round).error(R.mipmap.ic_launcher_round).bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
    }
